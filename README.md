...
│   ├── router.ts     единая точка входа, маршрутизация внутри
...
```bash
npm run smoke                        # 13 проверок serverless API
...
`api/router.ts` становится serverless-функцией, а `vercel.json` направляет в неё
все запросы на `/api/*`.
...
- **Маршрутизация — через rewrite, а не через catch-all файл.** Vercel
  сопоставляет `api/[...path].ts` только с **одним** сегментом пути, поэтому
  `/api/bookings/{id}` до функции не доходил и отдавался платформенной 404 — при
  этом `/api/bookings` работал исправно. Из-за этого `PATCH` и `DELETE` по
  идентификатору были недоступны в проде, хотя локально проходили все проверки.
  Решение: одна функция `api/router.ts` плюс явный rewrite
  `/api/:route*` → `/api/router?__route=:route*`; смоук-прогон проверяет
  многосегментный путь отдельно, чтобы поломка не вернулась.
- **Импорты в `api/` — только с расширением `.js`.** Vercel компилирует файлы
  по одному, без бандлинга, а Node ESM не разрешает пути без расширения.
